...
class DPFDLoss(nn.Module):
    """Dual-Path Feature Distillation (DPFD) Loss.

    创新点：将CWD和MGD两种蒸馏方法融合，通过自适应权重动态平衡：
    - CWD路径：捕获通道级语义分布知识（擅长类别识别）
    - MGD路径：捕获空间级结构生成知识（擅长缺陷定位）
    - 自适应注意力门控：根据特征图的空间注意力动态调整两条路径的权重

    相比单独使用CWD或MGD，DPFD能同时学习"看什么"和"在哪看"。
    """
    ...
    def forward(self, y_s, y_t):
        ...
        for idx, (s, t) in enumerate(zip(y_s, y_t)):
            ...
            # === 自适应门控权重 ===
            gate_input = t.detach()
            gate_weights = self.gate[idx](gate_input)  # (N, 2)
            w_cwd = gate_weights[:, 0].mean()  # CWD权重
            w_mgd = gate_weights[:, 1].mean()  # MGD权重
            ...
            # === MGD路径：掩码生成蒸馏 ===
            mat = torch.rand((N, 1, H, W), device=s.device)
            mat = torch.where(mat > 1 - self.lambda_mgd, 0, 1).float()
            masked_fea = torch.mul(s, mat)
            new_fea = self.generation[idx](masked_fea)
            mgd_loss = F.mse_loss(new_fea, t, reduction='sum') / N * self.alpha_mgd

            # === 自适应融合 ===
            combined_loss = w_cwd * cwd_loss + w_mgd * mgd_loss
...
class FeatureLoss(nn.Module):
    def __init__(self, channels_s, channels_t, distiller='mgd', loss_weight=1.0):
        ...
        if distiller == 'mgd':
            ...
            self.feature_loss = DPFDLoss(channels_s, channels_t)
        else:
